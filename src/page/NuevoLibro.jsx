import { agregarCliente } from "../data/LibrosApi"
import Formulario from "../components/Formulario"
import Error from "../components/Error"
import { useNavigate, Form, useActionData, redirect } from "react-router-dom"
// useNavigate Se usa para poder navegar en este caso hacia la pagina anterior
//el loader es una peticion de tipo get, El Form es un componente agregado en react-router-dom para el formulario y tambien existe una funcion para manejar las peticiones de tipo post

//Funcion para manejar las peticiones de tipo post
// En un actions siempre se va presentar un request que es una paticion que se realiza hacia el action
export async function action({ request }) {
    ...
    await agregarCliente(datos) // Ponemos un await para que no se ejecute hasta que finaliza la funcion de agregar cliente
    return redirect('/')
}
...
const NuevoLibro = () => {
    ...
    const errores = useActionData(); // useActionData lo usamos para pasar la informacion que hay en el action hacia los ocmponentes, es igual que loader con useLoaderData usamos estos hook para acceder a la informacion respectiva 
    const navigate = useNavigate();

    return (
        <>
            <h1 className="font-black text-4xl text-blue-900">Nuevo Libro</h1>
            <p className="mt-3">LLena todos los campos para registrar un nuevo libro</p>
